...
import pandas as pd
import os
csvfile = 'https://huggingface.co/spaces/Hermath/test2/blob/main/수학(상)_테스트_001_정답.csv'
...
def submit(*arg):
    ...
    if not os.path.exists('result.csv'):
        outputcsv.to_csv('result.csv',index=False, mode='w', encoding='utf-8-sig')
    else:
        outputcsv.to_csv('result.csv',index=False, mode='a', encoding='utf-8-sig', header=False)
